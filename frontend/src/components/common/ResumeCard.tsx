...
export function ResumeCard({ resume, onDuplicate, onDelete }: ResumeCardProps) {
  ...
  return (
    <article data-testid="resume-card" data-title={title} className="group flex min-h-[260px] flex-col justify-between border border-[var(--border)] bg-[var(--surface)] p-5 shadow-panel transition hover:-translate-y-0.5">
      <div>
        <div className="flex items-start justify-between gap-3">
          ...
          <Menu as="div" className="relative">
            <MenuButton data-testid="resume-menu" className="flex h-9 w-9 items-center justify-center rounded-md text-[var(--muted)] hover:bg-[var(--surface-alt)]">
              <MoreVertical size={18} aria-hidden />
            </MenuButton>
            <MenuItems className="absolute right-0 z-20 mt-2 w-44 border border-[var(--border)] bg-[var(--surface)] p-1 shadow-panel">
              <MenuItem>
                <button
                  data-testid="duplicate-resume"
